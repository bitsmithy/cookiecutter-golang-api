...
func (*server) writeJSON(w http.ResponseWriter, status int, data any, headers http.Header) error {
	js, err := json.Marshal(data)
	if err != nil {
		return err
	}

	js = append(js, '\n') // Newline for nicer terminal output
...
func (s *server) readJSON(w http.ResponseWriter, r *http.Request, dst any) error {
	maxBytes := 1_048_576
	return s.readJSONWithMaxBytes(w, r, maxBytes, dst)
}

//lint:ignore U1000 useful templating code, remove lint:ignore when using for the first time
func (s *server) readJSONWithMaxBytes(w http.ResponseWriter, r *http.Request, maxBytes int, dst any) error {
